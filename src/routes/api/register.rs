...
async fn register(body: web::Json<RegisterRequest>, pool: web::Data<PgPool>) -> impl Responder {
    ...
    let password_hashed = bcrypt::hash(&body.password, bcrypt::DEFAULT_COST)
        .expect("Error al hashear la contraseña");

    let user_id: (i32,) = sqlx::query_as("INSERT INTO users (email, password) VALUES ($1, $2) RETURNING id")
        .bind(&body.email)
